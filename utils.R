  ...
  released_t = case_when(
    ...
    mutate(released_test =  ifelse(type == "symptomatic" & 
           ...
           released_t = ifelse(released_test == "Symptomatic during quarantine",
                               flight_arrival + pmax(onset + post_symptom_window,
...
make_release_figure <- function(x,
  ...
  dy <- select(ungroup(x), !!!facet_vars,
               `97.5%`, `75%`) %>%
    ...
    distinct %>%
    {left_join(select(., -value) %>% as.list %>% map(unique) %>% expand.grid, .)} %>%
    spread(key, value) %>%
  ...
  ymax <- x %>% group_by(country) %>% 
    filter(is.finite(`97.5%`)) %>% 
    summarise(M = max(`97.5%`)) 
  ...
  mult <- c(0.1, ifelse(min(needs_expanding, TRUE), 0.25, 0.1))
...
fun.max.ci <- function(x){
  y <- data.frame(ymax = max(x[is.finite(x)]))
  y$ymin <- y$ymax
  y$y <- y$ymax
  return(y)
